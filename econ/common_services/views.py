...
from econ.common_services.models import FredUSRegConGasPrice, FredUSRegAllFormGasPrice, FredCrudeOilPrices, kQualsimAttempts, testdata
from econ.common_services.models import QLAssignmentAdditionalDetails
...
def getFredData(request):
    ...
    try:
        ...
        return HttpResponse(status=400)
    response = [[result[0].isoformat(), float(result[1])]
...
def initAttemptData(request):
    ...
    try:
        ...
        response = HttpResponse('Error: Init attempt:' + str(e) + trackdata, 404)
        return response
...
def getAttemptData(request):
    ...
    try:
        ...
        if hasattr(attempt, 'session_id'):
            ...
            # else condition is added to handle old data for review, before
            # multiple attempt logic implemented.
            try:
                ...
                if lstatedata.get('Attempts'):
                    ...
                    attIndex = kQualsimAttempts.objects.filter(
                        session_id=sessionId).count()

                    with connection.cursor() as cursor:
                        sql_query = """INSERT INTO common_services_kqualsimattempts 
                        (session_id, start_date, end_date, status, att_index, state_data) 
                        VALUES (%s,%s,%s,%s,%s, compress(%s))"""

                        cursor.execute(sql_query, (sessionId, str(datetime.datetime.today()), str((datetime.datetime.today(
                        ) + datetime.timedelta(seconds=duration))), 'inprogress', attIndex, attempt.problem_state_data))
                ...
                response = HttpResponse(
                    'Error: get session data:' + str(e), 404)
                return response
...
def getLastAttemptData(request):
    ...
    try:
        ...
        try:
            ...
            response = HttpResponse(
                'Error: get last attempt5:' + str(ine) + " funct erro:" + errormsg, 404)
            return response
    except Exception as e:
...
def getAllAttemptData(request):
	...
	sessionId = request.GET.get('sessionId', None)
        ...
        FROM common_services_kqualsimattempts WHERE session_id=%s"""

        if pstatus != 'all':
            sql_query = """SELECT id, session_id, start_date, end_date, status, att_index, uncompress(state_data) as state_data 
            FROM common_services_kqualsimattempts WHERE session_id=%s AND status=%s"""
            queryfilter = [sessionId, pstatus]

        attempts = kQualsimAttempts.objects.raw(
            sql_query, queryfilter)
...
def getQlSimShortData(request):
    ...
    try:
        ...
        package['Total_Completed'] = kQualsimAttempts.objects.filter(
            status='complete').count()
        package['Total_Inprogress'] = kQualsimAttempts.objects.filter(
...
def getQlSimIncorrectData(request):
    ...
    try:
        attempts = kQualsimAttempts.objects.filter(
            session_id__icontains=plcholder).order_by('session_id')
        lcl_list = []
        for attmpt in attempts:
            lstatedata = json.loads(attmpt.state_data)
            if lstatedata.get('Attempts'):
...
def updateQlSimIncorrectData(request):
    ...
    try:
        ...
        for attmpt in attempts:
            ...
            if lstatedata.get('Attempts'):
                if lstatedata['Attempts'][0]['reqdData'].get('bookmarkData'):
                    ...
                    if reqUpdate:
                        userScore = float(
                            lstatedata['Attempts'][0]['reqdData']['bookmarkData']['userScore'])
                        lstatedata['Attempts'][0]['overallScore'] = (
                            (userScore + 2) / maxScore * 100.0)
                        lstatedata['Attempts'][0]['reqdData']['bookmarkData']['userScore'] = (
                            userScore + 2)
        ...
        response = HttpResponse(
            'Error: updateQlSimIncorrectData:' + str(e), 404)
        return response
...
def postGradeQlSimIncorrectData(request):
    ...
    try:
        siddict = kQualsimAttempts.objects.filter(session_id__icontains=plcholder).order_by(
        ).values_list('session_id', flat=True).distinct()
        for sid in siddict:
            attmaxscoreacross = 0.0
            duration = 0
            reses = kQualsimAttempts.objects.filter(
                status='complete', session_id=sid).order_by('session_id')
            for res in reses:
                lstatedata = json.loads(res.state_data)
                if attmaxscoreacross <= float(lstatedata['Attempts'][0]['overallScore']):
                    attmaxscoreacross = float(
                        lstatedata['Attempts'][0]['overallScore'])
                    duration = lstatedata['Attempts'][0]['duration']
            ...
            targetpointstmp = launch_data['custom_points_' +
                                          launch_data['custom_currentquestion']]
            ...
            try:
                TPIUtils.submit_outcome(launch_data, problemNumber=problemNumber,
                                        problem_guid=problem_guid, score=score, duration=duration, submissionCount=2)
            except Exception as e:
                pass
        ...
        response = HttpResponse(
            'Error: postGradeQlSimIncorrectData:' + str(e), 404)
        return response
...
def updateQlSimIncorrectDataId(request):
    ...
    try:
        attempts = kQualsimAttempts.objects.filter(
            session_id=qssid).order_by('session_id')
        for attmpt in attempts:
            lstatedata = json.loads(attmpt.state_data)
            if lstatedata.get('Attempts'):
                if lstatedata['Attempts'][0]['reqdData'].get('bookmarkData'):
                    ...
                    if reqUpdate:
                        prevAttMaxScore = float(
                            lstatedata['Attempts'][0]['maxscore'])
                        lstatedata['Attempts'][0]['overallScore'] = (
                            userScore / (maxScore + 2)) * 100.0
                        if prevAttMaxScore > 1:
                            lstatedata['Attempts'][0]['maxscore'] = prevAttMaxScore - (
                                prevAttMaxScore - ((prevAttMaxScore / 100 * 38) / (38 + 2) * 100))
...
def postGradeQlSimIncorrectDataId(request):
    ...
    try:
        siddict = kQualsimAttempts.objects.filter(session_id=qssid).order_by(
        ).values_list('session_id', flat=True).distinct()
        for sid in siddict:
            attmaxscoreacross = 0.0
            duration = 0
...
def getQlSimIncorrectDataCount(request):
    ...
    try:
        attempts = kQualsimAttempts.objects.filter(
            session_id__icontains=plcholder)
        siddict = kQualsimAttempts.objects.filter(session_id__icontains=plcholder).order_by(
        ).values_list('session_id', flat=True).distinct()
...
def getQlSimDuplicateEntries(request):
    ...
    try:
        ...
        attempts = kQualsimAttempts.objects.filter(
            status=att_status).order_by(orderby)[sidx:eidx]
        lcl_list = []
        for attmpt in attempts:
            lstatedata = json.loads(attmpt.state_data)
            if lstatedata.get('Attempts'):
                if lstatedata['Attempts'][0]['reqdData'].get('bookmarkData'):
                    ...
                    if hasDup:
                        ...
                        package['status'] = attmpt.status
                        package['overallScore'] = 0
                        package['calculated_score'] = (
                            userScore / maxScore) * 100.0
                        attmaxscore = 0.0
                        try:
                            attmaxscore = float(
                                lstatedata['Attempts'][0]['maxscore'])
                        ...
                        attmaxscoreacross = 0.0
                        reses = kQualsimAttempts.objects.filter(
                            status='complete', session_id=attmpt.session_id)
                        for res in reses:
                            ...
                            if attmaxscoreacross <= float(lstatedataacross['Attempts'][0]['overallScore']):
                                if(float(lstatedata['Attempts'][0]['overallScore']) != float(lstatedataacross['Attempts'][0]['overallScore'])):
                                    attmaxscoreacross = float(
                                        lstatedataacross['Attempts'][0]['overallScore'])
        ...
        response = HttpResponse(
            'Error: getQlSimDuplicateEntries:' + str(e), 404)
        return response
...
def updateQlSimDuplicateEntries(request):
    ...
    try:
        ...
        response = HttpResponse(
            resptextstring + '---------Error: updateQlSimDuplicateEntries:' + str(e), 404)
        return response
...
def updateQuesAttemptDetails(request):
    ...
    try:
        ...
        knowdlAttIds = kam.ql_masterattempts.objects.filter(
            QL_Id__icontains=qlidlike).values("Id", "AssignmentLocation").all()
...
def gettableCount(request):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            strtrace = strtrace + " ----Key-value:" + \
                template_mappings["JWKS_URL"] + " ----temp:" + \
                template_mappings["38a1192c-778f-11e7-b5a5-be2e44b06b34"]
        else:
        ...
        # return
        # redirect('/knowdl_revel_sim/commonfiles/template_mappings.json')
...
# https://dev.econdip.pearsoncmg.com/econservice/data/econ/common_services/testmaria/
@csrf_exempt
@CorsHttpDecorator
def testmaria(request):
    #rawin = kQualsimAttempts.objects.using('optdb').raw("insert into testdata values('"+ cstr(datetime.datetime.now())  +"','compte')")
    '''
    with connection.cursor() as cursor:
	sql_query = """insert into testdata values(%s,'compte')"""
	cursor.execute(sql_query, (cstr(datetime.datetime.now()) ))
    '''
    tstdta = testdata.objects.using('optdb').create(session_id=str(datetime.datetime.now()), status='cm')
...
def GetAssignmentData(request):
    ...
    try:
        ...
        tracestr = tracestr + "BeginDate:" + \
            datetime.datetime.combine(
                begin_date, datetime.time.max).strftime("%Y-%m-%d %H:%M:%S")
        valueslist = kam.ql_masterattempts.objects.filter(StartDate__gt=datetime.datetime.combine(
            begin_date, datetime.time.max)).values("QL_Id", "Assignment_Id").all()
        ...
        assignments = kam.ql_assignmentdetails.objects.filter(
            Assignment_Id__in=assignidList, QL_Id__in=qlidlist).all()
        for asgn in assignments:
            try:
                ...
                tracestr = tracestr + "Error In Parsing Assignment Data: Assignment_Id" + \
                    asgn.Assignment_Id + "-Exception:" + str(innerex2)

        tracestr = tracestr + "Completed Fetching Assignment List-->"
        tracestr = tracestr + "Fetching Attempts List-->"
        attempts = kam.ql_masterattempts.objects.filter(
            StartDate__gt=datetime.datetime.combine(begin_date, datetime.time.max)).all()
        ...
        for att in attempts:
            ...
            try:
                ...
                try:
                    ...
                    tracestr = tracestr + "Error In Fetching Question Details: MstAttemptId=" + \
                        str(att.Id) + "Exception:" + str(innerex1)
            except Exception as innerex:
                tracestr = tracestr + "Error In Parsing Attempt: AttemptId" + \
                    str(att.Id) + "-Exception:" + str(innerex)
...
@csrf_exempt
@CorsHttpDecorator
def getClassAverage(request):
    if not request.method == 'GET':
        return HttpResponse(status=400)
    try:
        qlid = request.GET.get('qlid', '')
        asgnid = request.GET.get('asgnid', '')
        maxscore = float(request.GET.get('maxscore', -1))
        score = float(request.GET.get('score', -1))
	avgScore = 0.0;
        #avgScore = QLAssignmentAdditionalDetails.getClassAverageScore(qlid, asgnid, maxscore, score)
	sql_query = """SELECT Id, QL_Id, Assignment_Id, TotalScore FROM common_services_qlassignmentadditionaldetails
        WHERE QL_Id=%s and Assignment_Id=%s"""
        adddetails = QLAssignmentAdditionalDetails.objects.raw(sql_query, [qlid, asgnid])
        if(sum(1 for result in adddetails) > 0):
            if(score!=-1):  # user had better score or its first attempt
                if(maxscore==-1):
                    ...
                    SET TotalScore = (TotalScore + %s),  TotalUsers = (TotalUsers + 1) WHERE QL_Id=%s and Assignment_Id=%s"""
		    with connection.cursor() as cursor:
                    	cursor.execute(sql_updatequery, (score,  qlid, asgnid))
        ...
        #get average score
        sql_getquery = """SELECT Id, QL_Id, Assignment_Id, TotalScore, TotalUsers FROM common_services_qlassignmentadditionaldetails
        WHERE QL_Id=%s and Assignment_Id=%s"""
        scoredetail = QLAssignmentAdditionalDetails.objects.raw(sql_getquery, [qlid, asgnid])
	classavgscore = 0.0
        if(sum(1 for result in scoredetail) > 0):
            classavgscore = scoredetail[0].TotalScore/scoredetail[0].TotalUsers;

        return HttpResponse(classavgscore, 200)

    except Exception as e:
        response = HttpResponse('Error: get avg Score:' + str(e), 404)
        return response

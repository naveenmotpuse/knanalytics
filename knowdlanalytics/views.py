...
k_ANALYTICS_STATIC = '/content/staticfiles/knowdlanalytics/'
...
def fPostAttemptData(jsondatavalues, location):
    ...
    try:
        ...
        filter1 = {"Assignment_Id": assignmentId, "QL_Id": qlId,
                   "Student_Id": studentId, "CompletionStatus": "inprogress"}
        ...
        filterMaxScr = {"Assignment_Id": assignmentId, "QL_Id": qlId,
                        "Student_Id": studentId, "ReportStatus": "active", "CompletionStatus": "complete"}
        ...
        if qlma is not None:
            ...
            if completionStatus != "" and completionStatus.lower() == "complete":
                ...
                if qlma.ReportStatus == "active":
                    filterRStat = {"Assignment_Id": assignmentId,
                                   "QL_Id": qlId, "Student_Id": studentId}
                    m.ql_masterattempts.objects.filter(
                        **filterRStat).exclude(Id=qlma.Id).update(ReportStatus="inactive")
...
def AddQuestionDetails(qlma, jsondatavalues):
    ...
    if qId is not None and qId != "" and qlma is not None:
        try:
            ...
            '''
            filterQAtmpt = {"MstAttemptId": qlma.Id,
                            "PageId": pId, "QuestionId": qId}
            ...
            if qlqad is not None:
                ...
                qlqad = m.ql_questionattemptdetails.objects.create(MstAttemptId=qlma.Id, PageId=pId, QuestionId=qId, CorrectStatus=qCorrectStatus,
                                                                   TimeSpent=qTimespent, Score=qScore, Points=qPoints, SelOptionId=qSelOptionId, AdditionalInfo=str(qAdditionalInfo))
    ...
    try:
        ...
        if "NumberOfAttempts" in jsondatavalues:
            numberOfAttempts = int(
                str(jsondatavalues["NumberOfAttempts"]))  # int ??

        if "TargetPoints" in jsondatavalues:
            targetPoints = float(
                str(jsondatavalues["TargetPoints"]))  # double ??
...
def DBLog(msg, ex):
    logstr = msg
    if ex is not None:
        logstr = logstr + "==== ExceptionMessage:" + \
            str(ex) + ", ExceptionStacktrace:"
...
def QLSimAnalyticsDetails(request):
    ...
    if QL_Id != "" and ASGN != "":
        ...
        AssignmentTitle = m.ql_assignmentdetails.objects.filter(
            **filter1).values("AssignmentTitle").distinct()
    else:
        AssignmentTitle = ""
    ...
    for obj in j_objectives:
        ...
        for pg in j_pageids:
            ...
            if masterQ is not None and masterQ.count() > 0:
                ...
                filterScr = {"QuestionId": question.QuestionId,
        ...
        filterScr1 = {"QuestionId__in": pageids,
                      "MstAttemptId__in": mstattempids}
...
def GetLocationList():
    ...
    lstDIAD = m.ql_assignmentdetails.objects.values(
        'AssignmentLocation').distinct()
...
def GetAssignmentsString(request):
    ...
    filter1 = {"QL_Id": QL_Id}
    qlAssignments = m.ql_assignmentdetails.objects.filter(
        **filter1).values("Assignment_Id", "AssignmentTitle").order_by("AssignmentTitle").all()
        ...
        for asgmnt in qlAssignments:
            if optionString.find("value='" + asgmnt["Assignment_Id"] + "'") == -1:
                optionString = optionString + "<option value='" + \
                    asgmnt["Assignment_Id"] + "'>" + \
                    asgmnt["AssignmentTitle"] + "</option>"
...
def GetLocQLString(request):
    ...
    qlmediaarr = m.ql_assignmentdetails.objects.filter(
        **filter1).values("QL_Id", "QLTitle").order_by("QLTitle").all()
